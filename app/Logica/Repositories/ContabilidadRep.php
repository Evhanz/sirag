...
AND PERIODO=A.PERIODO
AND TIPO_COMPROBANTE='EGRESO'
AND CUENTA='040110104001') ITP )AS 'c11',
COALESCE(ROUND((select CASE WHEN MONEDA='S/.' THEN TOTALINGRESO ELSE TOTALINGRESO * (select valor from 
flexline.GEN_TASCAM
where EMPRESA = A.EMPRESA
AND MONEDA=C.MONEDA
AND FECHA=A.FECHA)
 END from flexline.Documento
where Empresa=a.EMPRESA
and TipoDocto=A.TIPO_DOCUMENTO
and Numero=A.REFERENCIA
AND CTACTE=A.AUX_VALOR2),2),ROUND((A.HABER_INGRESO/0.03),2)) AS 'c11_1',
...
and Numero=A.REFERENCIA
AND CTACTE=A.AUX_VALOR2
),(select CASE WHEN CUENTA='042010101001' THEN 'PEN' ELSE 'USD' END from flexline.CON_MOVCOM
where Empresa=A.EMPRESA
AND CORRELATIVO=A.CORRELATIVO
AND PERIODO=A.PERIODO
